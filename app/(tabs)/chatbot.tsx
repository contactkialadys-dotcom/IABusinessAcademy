...
// Backend removed for Vercel compatibility
...
export default function ChatbotScreen() {
  ...
  // Chatbot backend removed for Vercel compatibility
  // Using local mock responses instead
  ...
  const sendMessage = async (message: string) => {
    ...
    const assistantMessage: Message = {
      ...
      role: "assistant",
      content: "Désolé, le chatbot IA nécessite un backend serveur qui n'est pas disponible dans cette version statique. Pour activer le chatbot, vous devez déployer le projet avec un backend (Heroku, Railway, etc.). En attendant, vous pouvez consulter les formations et e-books disponibles ! 🚀",
      timestamp: new Date(),
    };
            ...
            <View className="flex-row items-center gap-2">
              ...
              <TouchableOpacity
                className={`rounded-2xl p-3 shadow-md ${
                  inputText.trim() && !isSending ? "bg-primary" : "bg-surface"
                }`}
                onPress={handleSend}
              ...
              >
                <IconSymbol
                  ...
                  color={inputText.trim() && !isSending ? "#F5F0E8" : "#C9B8A8"}
                />
              </TouchableOpacity>
            </View>
          </View>
